...
<div class="place"><span><i class="glyphicon glyphicon-th-list"></i>&nbsp;位置：</span>
    ...
    <div class="pull-right">
        <a class="btn btn-primary insetBtn" href="index.php?a=AdminUser&f=add" type="button"><i
...
<div class="wrap">
  <div class="search">
    <form action="index.php?a=AdminUser&do=search" method="post">
      用户名：
      <input type="text" name="search_where[uname][like]" size="12" value="<?php echo $search_where['uname']['like'];?>"  class="form-control searchInput" />
      用户角色：
      <select id="role" name="search_where[role_id][eq]" class="form-control selectSubmit">
        ...
        <option value="<?php echo $v['id'];?>" <?php if($search_where['role_id']['eq']==$v['id']){ ?>selected="selected"<?php } ?>><?php echo $v['title'];?></option>
        <?php } } ?>
      </select>
      ...
      <tbody>
        <?php if(is_array($list)){foreach($list as $key=>$vo){?>
          <tr class="datalist">
            ...
            <td onclick="selectEdit(<?php echo $vo['id'];?>,'role_id','index.php?a=AdminUser&f=update',this,'role');"><?php echo $role_name[$vo['role_id']];?></td>
            <td onclick="editNow(<?php echo $vo['id'];?>,'description','index.php?a=AdminUser&f=update',this);"><?php echo $vo['description'];?></td>
...
<?php } ?></td>
            <td align="center" class="action">
                <a class="btn btn-default btn-xs" href="<?php echo U(array('f'=>"auth",'user_id'=>"{$vo['id']}")); ?>" title="授权"><i class="fa fa-cog fa-fw"></i>授权</a>
    ...
    <div class="btn-group myoption">
      ...
      <select  class="form-control selectSubmit" id="selectSubmit">
        ...
        <?php if(!$select_option){ ?>
          <option value="index.php?a=AdminUser&f=status&status=1">启用</option>
          <option value="index.php?a=AdminUser&f=status&status=0">锁定</option>
        <?php } ?>
      </select>
    </div>
  </div>
