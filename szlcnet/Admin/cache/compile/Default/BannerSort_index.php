...
<div class="place"><span><i class="glyphicon glyphicon-th-list"></i>&nbsp;位置：</span>
  <div class="pull-left">
    <ul class="placeul">
      <li><a href="javascript:;">广告分类列表</a></li>
    </ul>
  </div>
    ...
    <table cellspacing="1" class="tablelist">
      ...
      <tbody>
        <?php if(is_array($list)){foreach($list as $key=>$vo){?>
          <tr>
            <td align="center"><input name="id[]" type="checkbox" value="<?php echo $vo['banner_sort_id'];?>" class="checkbox" /></td>
            <td align="center"><?php echo $vo['banner_sort_id'];?></td>
            <td dataType="require" style="font-weight:bold;" class="editTd" data="<?php echo $vo['banner_sort_id'];?>,banner_sort_name,index.php?a=BannerSort&f=update&name=banner_sort_id,<?php echo $id;?>,<?php echo $tableName;?>"><?php echo $vo['banner_sort_name'];?></td>
            <td class="editTd" data="<?php echo $vo['banner_sort_id'];?>,banner_sort_order,index.php?a=BannerSort&f=update&name=banner_sort_id,<?php echo $id;?>,<?php echo $tableName;?>"><?php echo $vo['banner_sort_order'];?></td>
            <td class="editTd" data="<?php echo $vo['banner_sort_id'];?>,banner_sort_content,index.php?a=BannerSort&f=update&name=banner_sort_id,<?php echo $id;?>,<?php echo $tableName;?>"><?php echo $vo['banner_sort_content'];?></td>
    ...
    <div class="btn-group myoption">
      ...
      <select  class="form-control selectSubmit" id="selectSubmit">
        ...
        <?php if(!$select_option){ ?>
          <option value="index.php?a=BannerSort&f=status&status=1">启用</option>
          <option value="index.php?a=BannerSort&f=status&status=0">锁定</option>
        <?php } ?>
      </select>
    </div>
  </div>
