...
<div class="place"><span><i class="glyphicon glyphicon-th-list"></i>&nbsp;位置：</span>
  ...
  <div class="pull-left">
    <ul class="placeul">
      <li><a href="javascript:;">新闻分类列表</a></li>
    </ul>
  </div>
  <div class="pull-right">
    <a class="btn btn-primary insetBtn" href="index.php?a=NewsSort&f=add" type="button"><i
    ...
    <table cellspacing="1" class="tablelist">
      ...
      <tbody>
        <?php if(is_array($list)){foreach($list as $key=>$vo){?>
          <tr>
            <td align="center"><input name="id[]" type="checkbox" value="<?php echo $vo['news_sort_id'];?>" class="checkbox" /></td>
            <td class="editTd" data="<?php echo $vo['news_sort_id'];?>,news_sort_id,index.php?a=NewsSort&f=update&name=news_sort_id,<?php echo $id;?>,<?php echo $tableName;?>"> <?php echo $vo['news_sort_id'];?></td>
            <td class="editTd" data="<?php echo $vo['news_sort_id'];?>,news_sort_name,index.php?a=NewsSort&f=update&name=news_sort_id,<?php echo $id;?>,<?php echo $tableName;?>"> <?php echo $vo['news_sort_name'];?></td>
            <td class="editTd" data="<?php echo $vo['news_sort_id'];?>,news_sort_order,index.php?a=NewsSort&f=update&name=news_sort_id,<?php echo $id;?>,<?php echo $tableName;?>"><?php echo $vo['news_sort_order'];?></td>
    ...
    <div class="btn-group myoption">
      ...
      <select  class="form-control selectSubmit" id="selectSubmit">
        ...
        <?php if(!$select_option){ ?>
          <option value="index.php?a=NewsSort&f=status&status=1">启用</option>
          <option value="index.php?a=NewsSort&f=status&status=0">锁定</option>
        <?php } ?>
      </select>
    </div>
  </div>
