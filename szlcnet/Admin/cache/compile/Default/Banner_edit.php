...
<div class="formbody">
  ...
  <form method="post" action="index.php?a=Banner&f=update"    class="ajax_form">
    <table cellspacing="1" class="myform">
      <tr>
        ...
        <td><select name="parent_id" class="form-control selectSubmit">
			...
			<option value="<?php echo $v['banner_sort_id'];?>" 
            <?php if($vo['parent_id']==$v['banner_sort_id']){ ?>selected="selected"<?php } ?>
            ><?php echo $v['banner_sort_name'];?>
            </option>
            <?php } } ?>
          </select></td>
      </tr>
      <tr>
        ...
        <td><input type="text" name="banner_name" size="25" value="<?php echo $vo['banner_name'];?>" class="form-control"  /></td>
      </tr>
      <tr>
        <td align="right">URL链接地址：</td>
        <td><input type="text" name="banner_url" value="<?php echo $vo['url'];?>" size="25" class="form-control" /></td>
      </tr>
      <tr>
...
<input type="hidden" name="<?php echo $file_name;?>" value="<?php echo $img_url;?>" id="hide_file" />
      ...
      <tr>
        ...
        <td><input type="text" name="banner_order" value="<?php echo $vo['banner_order'];?>" size="25" class="form-control" /></td>
      </tr>
      <tr>
        <td align="right">分类描述：</td>
        <td><textarea name="banner_content" cols="68" rows="6" class="form-control bootstrp_textarea top8"><?php echo $vo['banner_content'];?></textarea></td>
      </tr>
      <tr>
        <td align="right"><input type="hidden" name="id" value="<?php echo $vo['banner_id'];?>" />
