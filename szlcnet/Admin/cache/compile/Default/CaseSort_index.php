...
<div class="place"><span><i class="glyphicon glyphicon-th-list"></i>&nbsp;位置：</span>
    ...
    <div class="pull-left">
        <ul class="placeul">
            <li><a href="javascript:;">案例分类</a></li>
        </ul>
    </div>
    <div class="pull-right">
        <a class="btn btn-primary insetBtn" href="index.php?a=CaseSort&f=add" type="button"><i class="fa fa-pencil"></i>&nbsp;&nbsp;新增</a>
    </div>
</div>
        ...
        <table cellspacing="1" class="tablelist">
            ...
            <?php if(is_array($list)){foreach($list as $key=>$vo){?>
                <tr>
                    ...
                    <td class="editTd" data="<?php echo $vo['case_sort_id'];?>,case_sort_name,index.php?a=CaseSort&f=update&name=case_sort_id,<?php echo $id;?>,<?php echo $tableName;?>"><?php echo $vo['mark'];?><?php echo $vo['case_sort_name'];?></td>
                    <td class="editTd" data="<?php echo $vo['case_sort_id'];?>,alias,index.php?a=CaseSort&f=update&name=case_sort_id,<?php echo $id;?>,<?php echo $tableName;?>"><?php echo $vo['alias'];?></td>
                    <td class="editTd" data="<?php echo $vo['case_sort_id'];?>,case_sort_order,index.php?a=CaseSort&f=update&name=case_sort_id,<?php echo $id;?>,<?php echo $tableName;?>"><?php echo $vo['case_sort_order'];?></td>
                    <td class="editTd" data="<?php echo $vo['case_sort_id'];?>,case_sort_content,index.php?a=CaseSort&f=update&name=case_sort_id,<?php echo $id;?>,<?php echo $tableName;?>"><?php echo $vo['case_sort_content'];?></td>
    ...
    <div class="btn-group myoption">
      ...
      <select  class="form-control selectSubmit" id="selectSubmit">
        ...
        <?php if(!$select_option){ ?>
          <option value="index.php?a=CaseSort&f=status&status=1">启用</option>
          <option value="index.php?a=CaseSort&f=status&status=0">锁定</option>
        <?php } ?>
      </select>
    </div>
  </div>
