...
<div class="place"><span><i class="glyphicon glyphicon-th-list"></i>&nbsp;位置：</span>
  ...
  <div class="pull-right">
    <a class="btn btn-primary insetBtn" href="index.php?a=Basic&f=add" type="button"><i class="fa fa-pencil"></i>&nbsp;&nbsp;新增</a>
  </div>
</div>
<div class="rightinfo">

<div class="search">
        <form action="index.php?a=Basic&do=search" method="post">
            <select id="role" name="search_where[basic_one_id][eq]" class="form-control selectSubmit">
                ...
                <?php if($search_where['basic_one_id']['eq']==$v['basic_sort_id']){ ?>selected="selected"<?php } ?>
    ...
    <table cellspacing="1"  class="tablelist">
      ...
      <tbody>
        <?php if(is_array($list)){foreach($list as $key=>$vo){?>
          <tr>
            <td align="center"><input name="id[]" type="checkbox" value="<?php echo $vo['basic_id'];?>" class="checkbox" /></td>
            <td> <?php echo $vo['basic_id'];?></td>
            <td dataType="require" class="editTd" data="<?php echo $vo['basic_id'];?>,basic_name,index.php?a=Basic&f=update&name=basic_id,<?php echo $id;?>,<?php echo $tableName;?>"> <?php echo $vo['basic_name'];?></td>
            <td> <?php echo $vo['parent'];?></td>
            <td dataType="require" class="editTd" data="<?php echo $vo['basic_id'];?>,url,index.php?a=Basic&f=update&name=basic_id,<?php echo $id;?>,<?php echo $tableName;?>"> <?php echo $vo['url'];?></td>
            <td class="editTd" data="<?php echo $vo['basic_id'];?>,basic_order,index.php?a=Basic&f=update&name=basic_id,<?php echo $id;?>,<?php echo $tableName;?>"><?php echo $vo['basic_order'];?></td>
    ...
    <div class="btn-group myoption">
      ...
      <select  class="form-control selectSubmit" id="selectSubmit">
        ...
        <?php if(!$select_option){ ?>
          <option value="index.php?a=Basic&f=status&status=1">启用</option>
          <option value="index.php?a=Basic&f=status&status=0">锁定</option>
        <?php } ?>
      </select>
    </div>
  </div>
