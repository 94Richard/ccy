...
class HomeAction extends AdminAction
{
    ...
    public function right()
    {
        $info = array('操作系统' => PHP_OS,
            ...
            'phpinfo()' => '<a target="_blank" href="' . U(array(
                    'm' => 'Home',
                    'f' => 'myphpinfo'
                )) . '">查看</a>'
        );
        $proCount = $this->db->table("product")->where($this->lang)->count("product_id");
        $caseCount = $this->db->table("case")->where($this->lang)->count("case_id");
        $newsCount = $this->db->table("news")->where($this->lang)->count("news_id");
        $bannerCount = $this->db->table("banner")->where($this->lang)->count("banner_id");
        $msgCount = $this->db->table("message")->where($this->lang)->count("message_id");
        $count = array(
            "产品统计" => $proCount,
            "图片统计" => $caseCount,
            "新闻统计" => $newsCount,
