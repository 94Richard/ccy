...
class db
{
    ...
    public function __construct($config = array())
    {
        ...
        $this->config['DB_PCONNECT'] = isset($config['DB_PCONNECT']) ? $config['DB_PCONNECT'] : DB_PCONNECT; // true表示使用永久连接，false表示不使用永久连接
    ...
    public function connect()
    {
        ...
        if (! is_object($this->db)) {
            ...
            $this->conn = $this->db->getLink(); // 将对象返回给私有属性
        }
    }
    ...
    public function getAll($query)
    {
        ...
        while ($row = $this->db->fetch_array($query)) {
            // 为所有循环的数据。统一加一个IＤ。方便模板传值使用/.注意： 数据表的第一个字段必须为子增长ID。
            if ($i == 0) {
                
                $temp_arr = array_values($row);
                (is_numeric($temp_arr[0])) ? $row["url_id"] = $temp_arr[0] : '';
            }
            $data[] = $row;
    ...
    // 查询多条信息，返回数组
    public function select($is_select_all = true)
    {
        $table = $this->options['_table']; // 当前表
        $field = $this->options['_field']; // 查询的字段
        $field = $this->parseField($field);
