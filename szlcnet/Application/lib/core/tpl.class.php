 ...
 // 模板变量
    ...
    private function parse_tag_start($name)
    {
        ...
        if (! empty($matches[1])) {
            foreach ($matches[0] as $k => $v) {
                $params_arr = array(); // 参数
                /*
                 * preg_match_all('/\s(\w+)=[\"\'](.*?)[\"\']/',$matches[1][$k],$params); //参数值不能有单引号 preg_match_all('/\s(\w+)=[\"\'](\S*)[\"\']/',$matches[1][$k],$params); //参数值不能有空格 if(!empty($params[1])){ $size = sizeof($params[1]); for($i=0;$i<$size;$i++){ $params_arr[$params[1][$i]] = $params[2][$i]; } } // 参数值中有单引号，空格，以上有问题
                 */
    ...
    public function display($file, $ext = '.html')
    {
        if (! is_dir($this->tpl_cache)) {
            if (! @mkdir($this->tpl_cache, 0777))
                $this->throw_error('模板编译缓存目录' . $this->tpl_cache . '不可写！');
        }
        extract($this->tpl_var, EXTR_OVERWRITE);
        $compile_file = $this->tpl_cache . '/' . str_replace('/', '@', $file) . '.php';
        if (is_file($compile_file) && ! $this->debug)
            return include ($compile_file);
        $tpl_file =$this->tpl_dir . $file . $ext;
        
        if (! is_file($tpl_file))
            $this->throw_error('模板文件' . $tpl_file . '不存在！');
...
class tag_start
{
    ...
    public function _if($params = array())
    {
        ...
        foreach ($tmp as $search => $replace) { // condition="$v['credit'] gt 0"
            $params['condition'] = str_replace(' ' . $search . ' ', $replace, $params['condition']);
        }
