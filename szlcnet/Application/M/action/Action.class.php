...
class Action
{
    ...
    public function __construct()
    {
        ...
        $path_replace = array(
            ...
            1 => array(
                'search' => './css/',
                'replace' => TPL_PATH . 'css/'
            ),
            2 => array(
                'search' => './images/',
                'replace' => TPL_PATH . 'images/'
            ),
            3 => array(
                'search' => './img/',
                'replace' => TPL_PATH . 'img/'
            ),
            4 => array(
                'search' => '../common/',
                'replace' => APP_ROOT . 'template/common/'
            )
        );
        $this->tpl->tpl_replace = array_merge($this->tpl->tpl_replace, $path_replace);
        $PHP_SELF = $_SERVER['PHP_SELF'];
        // 网站URL根目录
        $this->url = str_replace("caradmin/", '', 'http://' . $_SERVER['HTTP_HOST'] . substr($PHP_SELF, 0, strrpos($PHP_SELF, '/') + 1));

        $this->assign("APP_URL", $this->url);
        // PC模板路径
        $this->assign("APP_THEME", TPL_PATH);
        //指定伪静态后缀
    ...
    protected function regex($value, $rule)
    {
        $validate = array(
            ...
            'integer' => '/^[-\+]?\d+$/',
            'double' => '/^[-\+]?\d+(\.\d+)?$/',
            "phone"=>"/^(((d{3}))|(d{3}-))?((0d{2,3})|0d{2,3}-)?[1-9]d{6,8}$/",
            'english' => '/^[A-Za-z]+$/',
            "chinese"=>'/^[\x{4e00}-\x{9fa5}]+$/u',
            'uname' => '/^[\x{4e00}-\x{9fa5}A-Za-z0-9_]+$/u',
            /* /^[".chr(0xa1)."-".chr(0xff)."A-Za-z0-9_]+$/   GB2312汉字字母数字下划线*/
        );
        // 检查是否有内置的正则表达式
