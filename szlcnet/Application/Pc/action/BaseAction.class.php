...
class BaseAction extends Action
{
    ...
    public function __construct()
    {
        ...
        // 获取网站配置表数据
        $this->webConfig = $this->db->table("web_config")->limit(0, 1)->find();
        $this->getMenu();
        $this->assign("config", $this->webConfig);
        $this->assign('defaultImg', $this->defaultImg);

        //获取阿里云的 oss 配置,cdn加速
        $oss = $this->db->table("aliyun_oss")->limit(0, 1)->find();
        if ($oss && count($oss)) {
            $this->assign("OSS_URL", $oss['url']);
        }
    }
    ...
    public function ajax_upload($file_type = 'Image')
    {
        if ($_FILES) {
            ...
            $AjaxUpload = new AjaxUpload($file_type, $_FILES[$file_name], array(
    ...
    public function getMenu()
    {
        ...
        //关于我们
        $this->assign("basic_about", $this->selectBasic(44,' and basic_id!=63'));
    ...
    //单页分类查询
    public function selectBasic($basic_one_id, $where = '')
    {
        return $this->db->table("basic")->where("basic_one_id=$basic_one_id {$where}")->order("basic_order desc")->select();
    }
