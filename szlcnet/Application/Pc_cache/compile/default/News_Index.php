...
<link rel="stylesheet" href="<?php echo $OSS_URL;?>css/News.css"/>
</head>
<body>
﻿<header id="header">
  <div class="headDiv">
...
<div class="subNav">
  <?php if(is_array($news_list)){foreach($news_list as $key=>$list){?> <a  
    <?php if($list['news_sort_id']==$curren_news){ ?>class="current"<?php } ?>
    href="<?php echo $APP_URL;?>News/Index/newslist/<?php echo $list['news_sort_id'];?>.html"><?php echo $list['news_sort_name'];?></a> <?php } } ?>
</div>
<div class="pageTitle">
  <h2>专注为中小型企业建设竞争力品牌官网</h2>
  <h3>Create value for customers,to ensure that customer satisfaction is our lifeline</h3>
</div>
<div class="pageContent">
  <div class="w1200 oh">
    <div class="newsList">
      <ul>
        <?php if(is_array($newsSet)){foreach($newsSet as $key=>$item){?>
          <li> <a href="<?php echo $APP_URL;?>news/Detail/id/<?php echo $item['news_id'];?>.html">
            <div class="dateL"><span><?php echo $item['day'];?></span><i><?php echo $item['news_date'];?></i></div>
            <div class="itemCtwo">
            <!--<img width="278" height="138" src="<?php echo $APP_URL;?><?php echo $item['news_spic'];?>"alt="<?php echo $item['news_title'];?>">-->
              <h2><?php echo $item['news_title'];?></h2>
              <p><?php chinesesubstr_admin($item['news_content'],0,260); ?></p>
            </div>
            </a> </li>
        <?php } } ?>
      </ul>
